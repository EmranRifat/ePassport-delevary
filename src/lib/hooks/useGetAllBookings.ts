...
import { GetAllBookingsRequest, AllBookingResponse } from "../types";
...
export const useGetAllBookings = ({ token }: { token: string | undefined }) => {
  ...
  const getAllBookings = async (
    requestData: GetAllBookingsRequest,
  ): Promise<AllBookingResponse> => {
    if (!token) {
    ...
    try {
      const response = await fetch(
        `${process.env.NEXT_PUBLIC_API_DMS_BASE_URL}/api/operator/paginationpassport`,
        {
          method: "POST",
          headers: {
      ...
      const normalizedResponse: AllBookingResponse = {
        ...
        passportissuedata: responseData.passportissuedata ?? [],
      };

      setData(normalizedResponse);
      return normalizedResponse;
    } catch (err: any) {
      const message = err?.message || "Failed to fetch booking list";
      setError(message);
      throw err;
    } finally {
      setLoading(false);
