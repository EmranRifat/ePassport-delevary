...
export const usePrintServerRes = () => {
  ...
  const submitPrintStatusServer = async (
    requestData: PendingBookingRequest,
    ...
    try {
      const response = await api.post<PendingBookingResponse>(
        `${process.env.NEXT_PUBLIC_API_DMS_BASE_URL}/api/operator/epassportpanding`,
        requestData,
        {
          headers: {
      ...
      if (!isSuccess) {
        const errorMsg =
          responseData.message || "Pending booking submission failed";
        setError(errorMsg);
